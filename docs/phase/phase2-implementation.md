...
- Phase 2A `script draft`: 只生成 beat-first `scene.py`，不渲染。
- Phase 2B `render implementation`: 在脚本草案通过本地静态分析后，继续渲染、修复并返回最终实现契约。
...
- Phase 2A system prompt: 来自 `prompts.get_phase2_script_draft_prompt()`。
- Phase 2B system prompt: 来自 `prompts.get_implementation_prompt()`。
- Phase 2A user prompt: 来自 `pipeline_phases12.build_phase2_script_draft_prompt()`。
- Phase 2B user prompt: 来自 `pipeline_phases12.build_implementation_prompt()`。
...
Phase 2A 使用 `prompts.get_phase2_script_draft_prompt()`，只描述脚本草案边界：

- 不重新规划。
- 不渲染、不运行 Manim、不轮询视频文件。
- 不做 TTS、mux、上传、render review 或最终用户总结。
- 只写 `scene.py`，并返回 `phase2_script_draft` structured output。
- 默认写 `GeneratedScene`。
- 必须使用 beat-first 代码结构。

Phase 2B 使用 `prompts.get_implementation_prompt()`，只描述渲染实现边界：
...
Phase 2A 和 2B system prompt 都约束可稳定渲染的生成方式：
...
Phase 2A user prompt 由 `build_phase2_script_draft_prompt()` 生成，只要求 Agent 写出结构完整的 `scene.py`：

- 不运行 Manim。
- 不创建视频。
- 不返回 `video_output`。
- 必须使用 `GeneratedScene`。
- 必须让 `construct()` 按 Phase 1 beat 顺序调用 beat 方法。
- 每个 beat 方法必须有足够的 `run_time` / `wait`，且末尾有完成态 hold。
- 每个 beat 的显式 `run_time + wait` 必须达到该 beat 目标时长的 80% 以上；
  Phase 2A 本地 gate 会用显式 timing 估算总时长，低于总目标 60% 时阻断。
- Phase 2A 不允许把 timing 不足写入 `deviations_from_plan` 后提交；如果自检低于任一门槛，必须先继续修改 `scene.py`，不能期望 Phase 2B 代为补足时长。

Phase 2B user prompt 由 `build_implementation_prompt()` 生成，负责传入任务实例数据：
...
Phase 2B prompt 会明确说明 `scene.py` 已经通过 Phase 2A 本地脚本草案 gate，要求 Agent 保留既有 beat-first 方法和 `construct()` 调用顺序，只继续做实现、渲染和必要修复。

## Skill 使用

Phase 2 不禁用 tools，也不设置 `skills=[]`。运行时通过 `ClaudeAgentOptions.plugins` 注入本地 `manim-production` plugin。

Phase 2A system prompt 只要求使用脚本草案相关的 stage cue：

1. `scene-build`: 根据 `build_spec` 写出 beat-first 脚本。
2. `scene-direction`: 组织视觉调度和节奏。
3. `layout-safety`: 避免明显布局重叠风险。

Phase 2A 不使用 `narration-sync` 或 `render-review`，因为它不生成 narration、不渲染视频。

Phase 2B system prompt 要求按以下顺序把 plugin skills 作为阶段 cue 使用：
...
Phase 2A Agent 只允许通过 SDK structured output 返回 `Phase2ScriptDraftOutput`。Pipeline 使用：

```python
PhaseSchemaRegistry.output_format_schema("phase2_script_draft")
```
...
- `scene_file`，必填
- `scene_class`
- `implemented_beats`，必填且非空
- `build_summary`，必填且非空
- `beat_timing_seconds`，必填且非空
- `estimated_duration_seconds`，必填
...
```text
backend/output/{task_id}/phase2_script_draft.json
backend/output/{task_id}/phase2_script_draft_analysis.json
```

Phase 2A 开始和通过本地脚本分析后都会发出 `phase="scene"` 的状态事件，分别标记：

- `Phase 2A script draft started. Writing beat-first scene.py before rendering.`
- `Phase 2A script draft accepted. Beginning render implementation pass.`

Phase 2B Agent 只允许通过 SDK structured output 返回 `Phase2ImplementationOutput`。Pipeline 使用：
...
进入 Phase 2A 前，dispatcher 设置：

```python
expected_output = "phase2_script_draft"
```

因此 Phase 2A 的 `ResultMessage.structured_output` 只按 `Phase2ScriptDraftOutput` 验证。
Phase boundary 使用 `phase_id="phase2a"`，用于单独统计脚本草案耗时。

进入 Phase 2B 前，dispatcher 设置：

```python
expected_output = "phase2_implementation"
```

Phase boundary 使用 `phase_id="phase2b"`，用于单独统计渲染实现耗时。

因此 Phase 2B 的 `ResultMessage.structured_output` 只按 `Phase2ImplementationOutput` 验证，不同时尝试：
...
- 每个 beat 方法的显式估算时长，并写入 `beat_duration_seconds`。

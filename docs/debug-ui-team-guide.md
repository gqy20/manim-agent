...
单个任务的 Debug 页只显示当前 task 绑定的 issue。如果要跨任务复盘历史问题，使用全局入口：

- 页面：`https://manim.gqy20.top/debug/issues`
- API：`GET /api/tasks/debug/issues`

全局列表会读取同一张 `debug_issues` 表，不会从 prompt artifacts、build summary 或 review blocking issues 自动生成记录。也就是说，只有在 Debug 页右侧保存过的 issue 才会出现在这里。
...
| --- | --- |
| `search` | 在标题和描述中模糊搜索 |
| `task_id` | 只看某个任务的 issue |
| `status` | 例如 `open`、`fixed`、`ignored` |
| `severity` | 例如 `low`、`medium`、`high`、`blocker` |
| `issue_type` | 问题分类，例如 `render`、`prompt`、`visual_quality` |
| `limit` | 返回数量，默认 100，最大 500 |

使用建议：

- 排查单个任务时，仍优先使用 `/tasks/{task_id}/debug` 右侧的 issue 区。
- 复盘重复问题时，使用 `/debug/issues` 搜索关键词或筛选分类，例如 `CJK`、`Axes`、`layout`、`tts`。
- 如果一个问题来自某个具体任务，仍然要保留 `task_id`，这样后续可以从全局列表跳回原始 Debug 页看 prompt、output 和上下文。
- 当前全局列表不需要数据库迁移；它复用现有 `debug_issues` 表，只是新增了不带 `task_id` 的查询入口。

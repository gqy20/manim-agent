...
Phase 1 system prompt 来自 `prompts.get_planning_prompt()`，负责定义阶段边界和 `phase1_planning` schema 契约。Phase 1 user prompt 来自 `pipeline_phases12.build_scene_plan_prompt()`，负责传入用户需求、目标时长和产品约束。

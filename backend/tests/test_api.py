...
from backend.models import ContentClarifyData, TaskCreateRequest, TaskStatus
...
class TestClarifyContent:
    def test_clarify_content_returns_structured_result(self, client):
        ...
        with patch("backend.routes.clarify_content", _fake_clarify):
            resp = client.post("/api/clarify-content", json={"user_text": "高斯定理"})

        assert resp.status_code == 200
        data = resp.json()
        assert data["original_user_text"] == "高斯定理"
        assert "clarification" in data
        assert data["clarification"]["topic_interpretation"].startswith("默认理解")
